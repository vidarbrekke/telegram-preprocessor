...
```javascript
import { preprocess } from "telegram-preprocessor"; // or "./index.mjs"

const { chunks, parseMode } = preprocess(agentReply, { style: "telegramHtml", split: true });
...
| `stripMetadata` | `false` | Set `true` to strip `message_id`/`sender_id`/`timestamp` from chunks, or pass a `RegExp` |
...
node test-proxy.mjs  # 8 proxy tests
```
